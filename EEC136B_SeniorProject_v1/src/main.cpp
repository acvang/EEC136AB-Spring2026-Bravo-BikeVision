...
static constexpr gpio_num_t PIN_HAZARD_CLEAR = GPIO_NUM_7;
...
static constexpr double HAZARD_CLEAR_RADIUS_M = 1000.0;
...
static constexpr gpio_num_t BRAKE_BUTTON_GPIO     = GPIO_NUM_9;
...
static constexpr float DECEL_BRAKE_TRIGGER_MPS2 = -0.05f;
// Try -0.20 for easier triggering, or -0.60 for harder braking only.

// Ignore tiny speed changes near 0 mph so the brake light does not flicker while stopped.
static constexpr float DECEL_BRAKE_MIN_SPEED_MPH = 0.0f;
...
static bool manual_brake_toggle_on = false;
...
static volatile float prev_speed_mps = 0.0f;
static volatile float speed_accel_mps2 = 0.0f;
...
static void i2c_bus_init()
{
    ...
    // Install the shared I2C bus once for VEML7700 and/or LIS3DH.
    // ESP_ERR_INVALID_STATE means the driver was already installed.
    esp_err_t err = i2c_driver_install(I2C_PORT, config.mode, 0, 0, 0);
...
// Updates the headlight, turn signals, speedometer, and automatic brake light.
static void bike_lights_update(Button &headlight_button, Button &left_button, Button &right_button, Button &brake_button, int64_t *last_blink_time_ms)
{
    const int64_t now = get_time_ms();

    // Press once = headlight ON, press again = headlight OFF.
    // The Button class debounces the active-low button and only returns true once per new press.
    if (headlight_button.pressed()) {
        headlight_on = !headlight_on;
    }

    // Manual brake override toggle.
    // Press once = brake light ON, press again = brake light OFF.
    // The Button class debounces the active-low brake button and only returns true once per press.
    if (brake_button.pressed()) {
        manual_brake_toggle_on = !manual_brake_toggle_on;
    }

    const bool manual_brake_active = manual_brake_toggle_on;
    ...
    // Headlight control.
    // Press button once to turn the headlight ON, press again to turn it OFF.
    // When ON and USE_VEML7700 is 1:
    // dark area = full brightness, bright area = dim brightness.
    if (!headlight_on) {
        ...
        if ((now - last_ambient_read_ms) >= AMBIENT_READ_INTERVAL_MS) {
            ...
            if (veml7700_ok && veml7700_read_lux(new_lux) == ESP_OK) {
                ...
                // Below 80 lux: full brightness.
                // Above 120 lux: dim brightness.
                // Between 80 and 120 lux: keep the previous brightness state.
    ...
    // Brake Light Control
    // The Hall sensor calculates speed_accel_mps2 in process_hall_pulse().
    // If speed_accel_mps2 is negative enough, the bike is slowing down, so turn on the brake light.
    // The brake button is also kept as a manual override for testing.
    const bool bike_is_moving = hall_speed_mph > DECEL_BRAKE_MIN_SPEED_MPH;
    const bool speed_is_decelerating =
        bike_is_moving && (speed_accel_mps2 <= DECEL_BRAKE_TRIGGER_MPS2);

    // Either trigger keeps the brake light on:
    // 1. Manual brake toggle is ON from pressing the brake button once.
    // 2. Hall speedometer detects enough negative acceleration.
    // The hold time keeps the automatic deceleration light from flickering between Hall pulses.
    if (speed_is_decelerating) {
        decel_brake_hold_until_ms = now + DECEL_BRAKE_HOLD_TIME_MS;
    }

    const bool auto_decel_brake_active = (now < decel_brake_hold_until_ms);
    const bool brake_light_on = manual_brake_active || auto_decel_brake_active;

    set_brake_brightness(brake_light_on ? BRAKE_FULL_DUTY : BRAKE_OFF_DUTY);

    if ((now - last_print_time_ms) >= BIKE_PRINT_INTERVAL_MS) {
        ESP_LOGI(TAG,
                 "brake speed=%.2f mph accel=%.2f m/s^2 decel=%d manual=%d auto=%d led=%d",
                 hall_speed_mph,
                 speed_accel_mps2,
                 speed_is_decelerating,
                 manual_brake_active,
                 auto_decel_brake_active,
                 brake_light_on);
...
extern "C" void app_main(void)
{
    ...
    // Start the shared I2C bus for the LIS3DH and/or VEML7700.
    i2c_bus_init();
#endif

#if USE_ACCELEROMETER
    // Only initialize the LIS3DH registers if the accelerometer is physically connected.
    lis3dh_init();
#endif

#if USE_VEML7700
    // Initialize the VEML7700 ambient light sensor for automatic headlight brightness.
    veml7700_init();
#else
    veml7700_ok = false;
#endif
